...
namespace FiiiChain.Miner.Android
{
    class Miner : IDisposable
    {
        ...
        public void InitPlotFiles()
        {
            ...
            var startIndex = Replot ? 0 : GetScoopStartIndex();
            ...
            for (int g = startG; g < groups; g++)
            {
                ...
                Task task = new Task(() =>
                {
                    this.SaveNonce(PlotFilePath, WalletAddress, copyData, max, (TotalCountofNonce - 1).ToString().Length);
                });
            }
        ...
        public int GetScoopStartIndex()
        {
            ...
            if (!scoopDirs.Any())
                return startScoop;
            var files = Directory.GetFiles(PlotFilePath + "/" + WalletAddress + "_4095");
            ...
            {
                var dirPath = Path.Combine(PlotFilePath, WalletAddress + "_" + i);
                if (!Directory.Exists(PlotFilePath) || !Directory.Exists(dirPath))
                {
                    return minNonce;
                }
                var noncefiles = Directory.GetFiles(WalletAddress + "_" + i.ToString("D4") + "_*");
        ...
        private void SaveNonce(string location, string address, NonceData[] nonceData, long maxNonce, int endNonceLen)
        {
            ...
            var minNonceText = ((maxNonce / noncePerFile) * noncePerFile).ToString("D5");
            var maxNonceText = maxNonce.ToString("D5");
            var fileName = string.Format("{0}_{1}", minNonceText, maxNonceText);

            Parallel.For(0, 4096, i =>
            {
                var dirName = Path.Combine(PlotFilePath, address +"_" + i);
                if (!Directory.Exists(dirName))
                {
                    Directory.CreateDirectory(dirName);
                }
                var path = Path.Combine(dirName, address + "_" + i.ToString("D4") + "_" + fileName);
