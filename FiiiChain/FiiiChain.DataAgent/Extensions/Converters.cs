...
namespace FiiiChain.DataAgent
{
    public static class Converters
    {
        ...
        public static List<PaymentCache> ConvertToSelfPayment(Transaction tx,out long amountChange)
        {
            amountChange = 0;
            var result = new List<PaymentCache>();
            var accounts = CacheManager.Default.GetAllKeys(DataCatelog.Accounts);
            ...
            var hasmyInputs = tx.Inputs.Where(x => accounts.Contains(x.AccountId));
            var hasOtherOuputs = tx.Outputs.Where(x => !accounts.Contains(x.ReceiverId));

            var subAmount = hasmyInputs.Sum(x => x.Amount);//input有自己的地址，减少
            var addAmount = tx.Outputs.Where(x => accounts.Contains(x.ReceiverId)).Sum(x => x.Amount);//output有自己的地址，添加
            amountChange = amountChange + addAmount - subAmount;

            var hasmyInput = hasmyInputs.Any();
            var hasOtherOuput = hasOtherOuputs.Any();
