...
using FiiiChain.PoolCenter.Helper;
...
namespace FiiiChain.PoolCenter
{
    public class PoolCenterJob
    {
        ...
        #region 生成区块
        public bool ForgeBlock(string startId, string minerAddress, long nonce)
        {
            var block = RedisManager.Current.GetDataInRedis<BlockMsg>(startId);
            if (block == null)
                return false;
            ...
            {
                //写信息到文件中
                FileCenterHelper.WriteBlockInfo(block);
                //关闭挖矿
                ...
                int generateBlockDelaySeconds = ConfigurationTool.GetAppSettings<PoolCenterSetting>("PoolCenterSetting").GenerateBlockDelayTime;
