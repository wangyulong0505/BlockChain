...
namespace FiiiChain.RabbitMQ
{
    public class RabbitMqClient
    {
        ...
        public static void ProduceMessage(List<string> messages)
        {
            ...
            factory.Uri = new Uri(RabbitMqSetting.TESTCONNECTIONSTRING);
            /*
        ...
        public static void ProduceMessage<T>(List<T> messages) where T : class
        {
            ...
            using (var connection = factory.CreateConnection())
            {
                using (var channel = connection.CreateModel())
                {
                    ...
                    //是否持久化
                    bool durable = true;
                    channel.QueueDeclare(RabbitMqName.FiiiPosInviteReward, durable, false, false, null);
                    if (messages != null && messages.Count > 0)
                    {
                        foreach (T message in messages)
                        {
                            ...
                            channel.BasicPublish("", RabbitMqName.FiiiPosInviteReward, properties, body);
