                ...
                */
                try
                {
                    ...
                    foreach (string tableName in rewardTables)
                    {
                        ...
                        //List<RewardList> tempRewardList = component.GetAllUnPaidRewardByAddresses(tableName, blockHashes, "'fiiimQmKV3A7HWfxfLRyxiGr8yEYVUar4Lw3Z6','fiiimDJ8sJYb38zJowrshQn8J1yzxgAeSKrWeQ', 'fiiimZSbHkjG5xq8uexEmQ8YPkxkBhxG8XFBcN', 'fiiimBrZUVjBrHK5CPFCh3t6W6gn2ueJE7iJg9'");
                    ...
                    if (rewardList != null && rewardList.Count > 0)
                    {
                        ...
                        //不够的话继续获取
                        ListPageUnspentOM unspent = await GetUnspentOM(currentPage, pageSize, testnet, tryTimes);
                        if(unspent == null)
                        {
                            return;
                        }
...
LabelSendItem:
                            ...
                            //添加服务费交易
                            receiverList.Add(new SendRawTransactionOutputsIM { Address = setting.ExtractReceiveAddress, Amount = Convert.ToInt64(Math.Floor(totalItemOriginAmount * setting.ServiceFeeProportion)) });
                            long totaloutputAmount = totalItemActualAmount + Convert.ToInt64(Math.Floor(totalItemOriginAmount * setting.ServiceFeeProportion));
                            ...
                            while (index < unspent.UnspentOMList.Count)
                            {
                                ...
                                {
                                    unspent = await GetUnspentOM(currentPage, pageSize, testnet, tryTimes);
                                    if(unspent == null)
                                    {
                                        return;
                                    }
                            ...
                            try
                            {
                                ...
                                RpcClient rpcClient = new RpcClient(new Uri(testnet ? "http://127.0.0.1:5006/" : "http://127.0.0.1:5007/"), auth, null, null, "application/json");
                                ...
                                RpcRequest request = RpcRequest.WithParameterList("SendRawTransaction", new List<object> { sendersList.ToArray(), receiverList.ToArray(), setting.ChangeAddress, 0, setting.FeeRate }, 1);
                            ...
                            catch(Exception exc)
                            {
                                ...
                                if(exc.Message.Contains("2020002"))
                                {
                                    ...
                                    unspent = await GetUnspentOM(currentPage, pageSize, testnet, 0);
                                    if(unspent == null)
                                    {
                                        return;
                                    }
                            ...
                            {
                                if (receiver.Address != setting.ExtractReceiveAddress)
                                {
                                    //LogHelper.Info($"send reward to {receiver.Address}, send amount is {receiver.Amount}");
                                    //根据address获取矿工信息
                            ...
                            foreach (var t in rewardTables)
                            {
                                ...
                                System.Threading.Thread.Sleep(setting.UpdateRewardSleepTime);
                            }
                            ...
                            */
                            System.Threading.Thread.Sleep(setting.CircleSleepTime);
                        }
                        //更新Blocks表中发送状态。
                        foreach (var t in rewardTables)
                        {
        ...
        static async Task<ListPageUnspentOM> GetUnspentOM(int currentPage, int pageSize, bool isTestNet, int tryTimes)
        {
            ...
            catch(Exception ex)
            {
                ...
                else
                {
                    ...
                    unspent = await GetUnspentOM(currentPage, pageSize, isTestNet, tryTimes);
                }
            }
            return unspent;
        }
